...
const char html[] = R"(
                  body {
                  background: linear-gradient(to bottom right, #8b00ff, #0084ff, #4e00ff, #a600ff, #0000ff);
                    }
                  form {
                       margin: 20px auto 0;
                       max-width: 500px;
...
const char config_js[] = R"rawliteral(
  ...
  <script>
          document.getElementById('switchjson').addEventListener('change', function() {
                      var status = this.checked ? 'on' : 'off';
                      var xhr = new XMLHttpRequest();
                      xhr.open('POST', '?page=indata', true);
                      xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
