void indata(){
 ...
 if(server.hasArg("IPMQTT") && server.hasArg("PASSWORD")){  
  ...
  if (server.hasArg("tube_config")){     
        settings.tubecof = server.arg("tube_config").toInt();
        save_config();
        String data  = "<text><b>Данные успешно сохранены!</b></text><br>";
               data += "<text><b>Установлен коэффициент трубки: "+String(settings.tubecof)+".</b></text>";
     server.send(200, "text/html", data);
  }
    if (server.hasArg("status")){  
    String msg;   
      if(server.arg("status")=="on"){
        settings.buzzer_en = true;
        save_config();
        }else if(server.arg("status")=="off"){
        settings.buzzer_en = false;
        save_config();
    } 
    server.send(200, "text/html", String(server.arg("status").c_str()));
  }
}
